...
export default function MovieCard({ movie }) {
  ...
  return (
    <div className="movie-card rounded-2xl overflow-hidden h-full flex flex-col">
      ...
      <Link to={`/MovieDetails/${id}`} className="pointer">
        <div className="movie-card-img-wrapper relative overflow-hidden bg-gray-900 aspect-[2/3] group flex-shrink-0">
          ...
          {/* Overlay */}
          <div className="movie-card-overlay absolute inset-0 w-full h-full flex flex-col items-center justify-end pb-4 md:justify-center md:pb-0 bg-transparent md:bg-gradient-to-t md:from-gray-900 md:via-gray-900/80 md:to-transparent opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-300 z-20">
            <div className="overlay-content text-center px-4">
              {/* Play Button - Desktop only */}
              <Link
                to={`/notfound/${id}`}
                className="btn btn-play bg-linear-to-r from-blue-500 to-cyan-500 text-white rounded-full p-4 hidden md:flex items-center justify-center hover:from-blue-600 hover:to-cyan-600 transition-all duration-300 shadow-lg mx-auto mb-4 transform hover:scale-110"
